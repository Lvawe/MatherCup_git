...
# 重新分配货物流量函数
def redistribute_cargo(data_original, dc_node, dc_neighbors, node_weights):
    ...
    for index, row in data_original.iterrows():
        if row["spot1"] == dc_node:
            ...
            for neighbor in dc_neighbors[row["spot2"]]:
                weight = node_weights[neighbor]  # 获取节点的权重
                redistributed_amount *= weight  # 根据节点权重调整分配数量
                new_row = pd.Series(
...
# 使用 GCN 模型预测节点权重
with torch.no_grad():
    pred_weights = model(data).flatten()
    node_weights = {node: weight.item() for node, weight in zip(nodes, pred_weights)}

# 重新分配货物流量
redistributed_cargo = redistribute_cargo(data_original, dc_node, dc_neighbors, node_weights)
