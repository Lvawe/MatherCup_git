...
for epoch in range(200):
    optimizer.zero_grad()
    out = model(data)
    loss = F.mse_loss(out, target)  # 确保提供正确的目标张量
    loss.backward()
    optimizer.step()
...
# 重新分配
def redistribute_dc5_cargo(data_original, dc5_neighbors):
    redistributed_data = data_original.copy()
    new_rows = []
    for index, row in data_original.iterrows():
        if row["spot1"] == "DC5":
            redistributed_amount = row["amount"] / len(dc5_neighbors[row["spot2"]])
            for neighbor in dc5_neighbors[row["spot2"]]:
                new_row = pd.Series({"spot1": neighbor, "spot2": row["spot2"], "date": row["date"], "amount": redistributed_amount})
                new_rows.append(new_row)
        elif row["spot2"] == "DC5":
            redistributed_amount = row["amount"] / len(dc5_neighbors[row["spot1"]])
            for neighbor in dc5_neighbors[row["spot1"]]:
                new_row = pd.Series({"spot1": row["spot1"], "spot2": neighbor, "date": row["date"], "amount": redistributed_amount})
                new_rows.append(new_row)
...
dc5_edges = data_original[(data_original["spot1"] == "DC5") | (data_original["spot2"] == "DC5")]
dc5_neighbors = {node: [n for n in nodes if n != "DC5" and n != node] for node in nodes}

redistributed_cargo = redistribute_dc5_cargo(data_original, dc5_neighbors)
